...
# Пробрасывает веб-порт для доступа к Flask приложению
...
case "${1:-}" in
    ...
    status)
        if check_tunnel_process; then
            ...
            log_message "Туннель работает: $info"
            echo "$info"
            exit 0
        else
...
if check_tunnel_process; then
    ...
    if check_web_tunnel_health; then
        info=$(get_current_info)
        log_message "Туннель работает нормально: $info"
        exit 0
    else
